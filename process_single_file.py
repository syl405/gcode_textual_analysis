...
def process_single_file(fs):
	...
	motion_params ={'X': 0, #current x position in mm
					...
					'A': 0, #current acceleration in mm/s^2
	...
	xyef_opts = {'X': lambda axis_str:float(axis_str[1:-1]),
				 ...
				 'Z': lambda axis_str:float(axis_str[1:-1]),
	...
	while cur_line: #Exit loop when cur_line is empty, i.e. EOF
		if cur_line.startswith(';'):
			...
			for i in match_lists:
				cur_axis = match_lists[i][0] #first character in string
				cur_axis_dest = float(match_lists[i][1:])
			...
			if cur_move_dist > motion_params['F']^2 / (2*motion_params['A']): #trapezoidal profile
			...
			motion_params['T'] = temp_regex.search(cur_line).group(1)
			...
			output['total_dwell_time'] += dwell_regex.search(cur_line).group(1)
